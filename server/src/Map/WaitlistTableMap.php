...
/**
 * This class defines the structure of the 'waitlist' table.
 *
 *
 *
 ...
 *
 */
class WaitlistTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = '.Map.WaitlistTableMap';
     ...
     */
    const OM_CLASS = '\\Waitlist';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'Waitlist';
     ...
     */
    const COL_USER = 'waitlist.user';

    /**
     * the column name for the shift field
     */
    const COL_SHIFT = 'waitlist.shift';

    /**
     * the column name for the event field
     */
    const COL_EVENT = 'waitlist.event';
  ...
  array (
    ...
    {
        return $withPrefix ? WaitlistTableMap::CLASS_DEFAULT : WaitlistTableMap::OM_CLASS;
    }
     ...
     * @return array           (Waitlist object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = WaitlistTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = WaitlistTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + WaitlistTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = WaitlistTableMap::OM_CLASS;
            /** @var Waitlist $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            WaitlistTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = WaitlistTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = WaitlistTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(WaitlistTableMap::COL_USER);
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(WaitlistTableMap::DATABASE_NAME)->getTable(WaitlistTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(WaitlistTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(WaitlistTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new WaitlistTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a Waitlist or Criteria object OR a primary key value.
     *
     ...
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            ...
            foreach ((array) $values as $singleval) {
                WaitlistTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return WaitlistQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a Waitlist or Criteria object.
     *
     * @param mixed               $criteria Criteria or Waitlist object containing data that is used to create the INSERT statement.
...
//
WaitlistTableMap::buildTableMap();
